...
class TBE_browser_recordList extends localRecordList {
	...
	function linkWrapItems($table,$uid,$code,$row)	{
		...
		$ficon = $BACK_PATH.t3lib_iconWorks::getIcon($table,$row);
...
class TBE_PageTree extends localPageTree {
	...
	function wrapTitle($title,$v,$ext_pArrPages)	{
		global $BACK_PATH;
		if ($ext_pArrPages)	{
			$ficon=$BACK_PATH.t3lib_iconWorks::getIcon('pages',$v);
...
RTE.default.linkhandler {
	tt_news {
		...
		$this->doc->inDocStyles .= 'html,body{overflow:scroll}';
		...
		if ($expPageId && t3lib_div::testInt($expPageId) && $BE_USER->isInWebMount($expPageId))	{
			...
			foreach ($queries as $table => $query) {
				...
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
								$query['select'],
								$currentTable,
								'sys_language_uid = 0 AND pid='.intval($expPageId).
									t3lib_BEfunc::deleteClause($currentTable),
								'',
								$query['sorting']
							);
